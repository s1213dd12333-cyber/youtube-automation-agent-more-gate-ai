...
assert(index.includes("const result = await this.startGenerationJob({ topic, style, length, source: 'manual' });"), 'manual API path must identify itself as manual');
assert(index.includes('res.status(error.status || 500).json({ success: false, error: error.message });'), 'manual generation API must catch startGenerationJob errors instead of letting them escape Express');
