...
function patchProductionManagement() {
  ...
  s = replaceOnce(
    ...
    `      // Keep a single in-memory entry when a failed production resumes under its stable id.\n      const pipelineIndex = this.pipeline.findIndex(item => item.id === productionId);\n      if (pipelineIndex >= 0) this.pipeline[pipelineIndex] = productionData;\n      else this.pipeline.push(productionData);`,
    'deduplicate resumed production in memory'
  );
