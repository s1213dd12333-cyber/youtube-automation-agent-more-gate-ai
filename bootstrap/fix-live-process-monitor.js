...
function patchRuntime() {
  ...
  const route = [
    ...
    "        const result = await this.processMonitor.snapshot({ limit: req.query?.limit });",
...
function patchDashboardJs() {
  ...
  const functionsBlock = [
    ...
    "  try {",
    "    ui.processData = await api('/api/process-monitor?limit=500');",
    ...
    "  $('#process-log-path').textContent = data.logPath || 'logs/combined.log';",
    "",
...
function patchStyles() {
  ...
  const styles = [
    ...
    '.process-toolbar { display: grid; grid-template-columns: 130px 190px minmax(220px,1fr) auto; gap: 10px; align-items: end; padding: 14px; border-bottom: 1px solid var(--line); }',
